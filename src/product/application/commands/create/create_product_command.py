...
from product.application.errors.name_already_exists import product_name_already_exists_error

class CreateProductCommand(IApplicationService):

    def __init__(self, id_generator: IDGenerator, product_repository: IProductRepository):
        self.id_generator = id_generator
        self.product_repository = product_repository

    async def execute(self, data: CreateProductDto) -> CreateProductResponse:

        product_by_name = await self.product_repository.find_by_name(name=ProductName(data.name))
        ...
        return Result.success(
            value=CreateProductResponse(id=product.id),
            info=product_created_info()
        )
